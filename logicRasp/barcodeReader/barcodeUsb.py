        ...
        while True:
            ...
            for char_code in [element for element in fp.read(8) if element > 0]:
                if char_code == CR_CHAR:
                    ...
                    label.value = barcode_string_output
...
def getBarcode(timeOut):
    manager = Manager()
    label = manager.Value('c','None')
    process = Process(target=barcode_reader,args=(label,))
    process.start()
